    ...
    router(obj) {
        switch (obj.method) {
            ...
            case 'mining.set_target':
                ...
                break;
            default:
                //Forward message to the correct miner
...
class CryptoNightPoolConnector extends PoolConnector {
    onConnect(connect) {
        logger.log("Connected to pool " + this.current_pool.host + ":" + this.current_pool.port);
        if (this.miners) {// Reset workers connections
            logger.warn('Resetting workers...');
            ...
            logger.warn('Creating workers listner...');
            this.miners = new minersController.CryptoNightMinersController(this, this.config);
        }
        this.login();
    }
    login() {
        // Subscribe to pool
        logger.log('login to pool...');
        var sub = {
            id: 1
            , method: "login"
            , params: {
                login: this.config.wallet + "." + this.config.proxy_name,
    ...
    onData(obj) {
        ...
        if (this.poolSocket != null) {
            switch (this.status) {
                case 'logining':// logining (internal)
                    if (obj.id == 1) {
                        if (obj.error) {
                            this.error(obj.error);
                        } else {
                            this.last_job = obj.result.job;
                            this.sessionId = obj.result.id;
                            this.last_target = this.last_job.target;
                            this.miners.broadcastToMiners(obj);
                            this.status = "ready";
                        }
                    }
